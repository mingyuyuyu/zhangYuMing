			...
			var carousel = {
				init:function(){
					...
					if(opt.showPage){
						...
						// 写上页码
						$page.children('span').each(function(idx,ele){
							$(this).html(idx+1);
						})
				...
				move:function(){
					...
					// 淡入淡出
						...
						$ul.children().eq(opt.index).stop().animate({opacity:1,},0).siblings('li').stop().animate({opacity:0},0);
						if(opt.index==0){
							$this.parent('.banner').css({'backgroundColor':'#EAC328'})
						}
						if(opt.index==1){
							$this.parent('.banner').css({'backgroundColor':'#B9EEF2'})
						}
						if(opt.index==2){
							$this.parent('.banner').css({'backgroundColor':'#FAD3C4'})
						}
						if(opt.index==3){
							$this.parent('.banner').css({'backgroundColor':'#A9DBFF'})
						}
						if(opt.index==4){
							$this.parent('.banner').css({'backgroundColor':'#E4F2F2'})
						}
						if(opt.index==5){
							$this.parent('.banner').css({'backgroundColor':'#006D74'})
						}
					}
